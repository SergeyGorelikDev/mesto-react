...
function Main(props) {
    ...
    function getInformation() {
        ...
        .then((dataFromSecondPromise) => {
         getCards(dataFromSecondPromise.map((item, key) => {
            return (
                <article className="element" key={key}>  
                    <Card cards = {item} onCardClick = {props.onCardClick}/>
                </article>
            );
        }))
    ...
    return (
        ...
        <PopupWithForm name="edit-profile" title="Редактировать профиль" children={editProfile} isOpen = {props.isEditProfilePopupOpen} onClose = {props.onClose}/>
        ...
        <PopupWithForm name="add-photo" title="Новое место" children={addPhoto} isOpen = {props.isAddPlacePopupOpen} onClose = {props.onClose}/>
        <PopupWithForm name="update-avatar" title="Обновить аватар" children={updateAvatar} isOpen = {props.isEditAvatarPopupOpen} onClose = {props.onClose}/>
